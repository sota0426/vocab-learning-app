...
export default function Quiz_1({ onBackToHome, onQuizStart }: QuizDisplayProps) {
  ...
  const generateChoices = () => {
    ...
    while (randomChoices.length < 3) {
      ...
      const randomWord = sameClassWords[randomIndex].word_1_ja;
      if (randomWord !== currentWordData.word_1_ja && !randomChoices.includes(randomWord)) {
        randomChoices.push(randomWord);
      }
    }
    const allChoices = [...randomChoices, currentWordData.word_1_ja].sort(() => Math.random() - 0.5);
  ...
  const handleChoiceClick = (choice: string) => {
    if (choice === currentWordData.word_1_ja) {
      setFeedback('正解です！');
      setShowFeedbackOverlay('correct');
    } else {
      setFeedback(currentWordData.word_1_ja);
  ...
  return (
    <div className="relative flex flex-col md:flex-row justify-center items-center h-full space-y-4 md:space-y-0 md:space-x-6 mb-6">
      ...
      <div className="flex flex-col items-center justify-center md:w-4/12">
        <p className="text-6xl font-bold mb-6">{currentWordData.word_1_en}</p>
        <AudioPlayer
          src={getAudioSource()}
          playbackRate={1}
          isPlaying={isPlaying}
        ...
        {isQuizFinished && (
          <div className="mt-6">
            ...
            <button
              onClick={() => navigate('/quiz_2')} // クイズ2へのナビゲート
              className="block w-full text-xl p-4  bg-green-500 text-white rounded mb-6 hover:bg-green-600 transition-all"
            >
              クイズ2へ進む
            </button>
